"""Flask-based mock Blackboard REST API server (latest API)."""
...
def create_app():
    ...
    enrollments_data = load_fixture("enrollments.json")  # dict course_id -> list
    assignments_data = load_fixture("assignments.json")  # dict course_id -> list
    ...
    def get_courses_v3():
        ...
        if user_id:
            enrolled_course_ids = []
            for course_id, enrollments in enrollments_data.items():
    ...
    # ---------- ASSIGNMENTS (list/update v1, create via contents) ----------
